...
// 设置是否可用
function setFlagSelect(vue, name) {
  for(var x in vue.flagOptions) {
    if (vue.flagOptions[x].name === name) {
      vue.postForm.flag = { key: vue.flagOptions[x].key, name: vue.flagOptions[x].name }
    }
  }
}
</script>
